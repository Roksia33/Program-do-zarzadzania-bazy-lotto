...
def save_to_database():
    try:
        ...
        e_numbers = [e_number_entry[i].get() for i in range(1)]
        date = entry_date.get()
        ...
        cursor.execute("INSERT INTO lotto_results (lotto_type, number1, number2, number3, number4, number5, e_number1, date, time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
# Tworzenie okna
root = tk.Tk()
root.title("Ekstra Pensja")
root.geometry("910x250")
...
lotto_type_var.set("Ekstra Pensja")
...
tk.Label(root, text="Liczby (1-35):", font=("Helvetica", 14)).grid(row=2, column=0)
number_entries = []
for i in range(5):
    entry = tk.Entry(root)
...
tk.Label(root, text="Liczba dodatkowa (1-4):", font=("Helvetica", 14)).grid(row=3, column=0)
e_number_entry = []
for i in range(1):
...
label1 = tk.Label(root, text="", font=("Helvetica", 10)).grid(row=9, column=3)
button_refresh = tk.Button(root, text="Reset", height = 2, width = 10, bg='#6aad32', command=refresh_data, font=("Helvetica", 10))
button_refresh.grid(row=7, column=2)
save_button = tk.Button(root, text="Zapisz",height = 2, width = 10, bg='#6aad32', command=save_to_database, font=("Helvetica", 10))
save_button.grid(row=7, column=3)
button_exit = tk.Button(root, text="Wyjście",height = 2, width = 10, bg='#6aad32', command=exit_program, font=("Helvetica", 10))
